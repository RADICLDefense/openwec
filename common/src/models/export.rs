...
pub fn serialize(subscriptions: &[crate::subscription::SubscriptionData]) -> Result<String> {
    let export = ImportExport::V4(subscriptions.into());
...
pub fn parse(content: &str) -> Result<Vec<crate::subscription::SubscriptionData>> {
    ...
    let subscriptions = match import {
        ...
        ImportExport::V3(subscriptions) => subscriptions
            .try_into()
            .context("Invalid subscription data")?,
        ImportExport::V4(subscriptions) => subscriptions
            .try_into()
            .context("Invalid subscription data")?,
    };
    Ok(subscriptions)
...
pub mod v4 {
    ...
    #[strum(ascii_case_insensitive)]
    pub(super) enum MachineIdentityStrategy {
        #[default]
        Subject,
        MachineID,
        SubjectAndMachineID,
    ...
    impl From<crate::subscription::MachineIdentityStrategy> for MachineIdentityStrategy {
        fn from(value: crate::subscription::MachineIdentityStrategy) -> Self {
            match value {
                crate::subscription::MachineIdentityStrategy::Subject => {
                    MachineIdentityStrategy::Subject
                }
    ...
    pub(super) struct SubscriptionData {
        ...
        pub filter: Option<ClientFilter>,
        pub outputs: Vec<SubscriptionOutput>,
        // New in V4: per-subscription identity strategy.
        // Default for back-compat is Subject.
        #[serde(default)]
        pub client_identity_strategy: MachineIdentityStrategy,
        #[serde(default)]
    ...
    impl TryFrom<SubscriptionData> for crate::subscription::SubscriptionData {
        ...
        fn try_from(value: SubscriptionData) -> Result<Self, Self::Error> {
            ...
            data.set_uuid(crate::subscription::SubscriptionUuid(value.uuid))
                ...
                .set_outputs(outputs?)
                .set_revision(value.revision)
                .set_client_identity_strategy(value.client_identity_strategy.into())
                .set_client_identity_fallback_strategy(
                    value.client_identity_fallback_strategy.map(Into::into),
                );
    ...
    impl From<crate::subscription::SubscriptionData> for SubscriptionData {
        fn from(value: crate::subscription::SubscriptionData) -> Self {
            ...
            Self {
                ...
                filter: value.client_filter().cloned().map(Into::into),
                outputs: value.outputs().iter().map(|o| o.clone().into()).collect(),
                client_identity_strategy: value.client_identity_strategy().into(),
                client_identity_fallback_strategy: value
...
mod tests {
    ...
    fn test_export_import() -> Result<()> {
        ...
        // Sanity-check we are now exporting using the V4 schema
        assert!(
            content.contains("\"schema\":\"V4\""),
            "expected V4 schema in exported content, got: {content}"
        );
    ...
    #[test]
    fn test_import_v3_defaults_identity_strategy_to_subject() -> Result<()> {
        // A V3-formatted export must still import successfully and default the
        // newly added identity strategy fields.
        let v3_payload = r#"{
            "schema": "V3",
            "data": {
                "subscriptions": [
                    {
                        ...
                        "outputs": [
                            {
                                "format": "Raw",
                                "driver": {"Files": {"path": "/tmp/{client}/messages"}},
                                "enabled": true
                            }
                        ]
                    }
        ...
        let imported = parse(v3_payload)?;
        assert_eq!(imported.len(), 1);
        let sub = &imported[0];
        assert_eq!(sub.name(), "legacy-sub");
        assert_eq!(
