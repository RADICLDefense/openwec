...
| `machine` | The effective per-machine identity computed from the subscription's `client_identity_strategy` (see [Per-machine identity strategy](subscription.md#per-machine-identity-strategy)). With the default `Subject` strategy this is identical to `{client}`. With `MachineID` or `SubjectAndMachineID`, this lets you store events from each physical machine in a dedicated path even when several machines share a credential. Same character whitelist as `{client}`. |
